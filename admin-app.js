(function(){
  ...
  q('memberForm').addEventListener('submit', function(event){
    ...
    const member = { id: uid('m'), name, email, phone:'', plan: plan.name, durationMonths: plan.durationMonths, membershipStatus:'active', joinDate: joinDate, renewalDate: renewalDate, trainerId: data.trainers[0] ? data.trainers[0].id : '', goals:['General fitness'], notes:`Paid by ${q('mPaymentMode').value}`, biometricId:`BIO-${Math.floor(Math.random() * 9000) + 1000}` };
    ...
    data.finance.push({ id: uid('f'), memberId: member.id, type:'income', category:'membership', amount: plan.price, desc:`${plan.name} signup`, date: today(), status:'paid' });
    q('memberForm').reset();
    save();
    renderAll();
  });
